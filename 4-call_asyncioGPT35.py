...
async def working(type_name, step, id_A_list, id_B_list, id_C_list, prompt_list, outline_list):
    ...
    # for i in range(0,8,step):
        ...
        os.makedirs(f'GPT35/COT/temp_{str(Temperature).replace(".","_")}_results', exist_ok=True)
        await save_responses_to_jsonl(paired_responses, f'GPT35/COT/temp_{str(Temperature).replace(".","_")}_results/{type_name}.jsonl')
        if i % 100 == 0:
